...
⚠️ **Важно для бесплатного тарифа Render:**

На бесплатном тарифе Render сервисы останавливаются после ~60 секунд бездействия. Это может вызывать:
- 502 ошибки при первом запросе после пробуждения
- Задержки до 30-60 секунд при холодном старте
- Проблемы с длительными операциями (например, подключение к Telegram)

### Рекомендуемые решения:

1. **Используйте внешний keepalive сервис** (бесплатно):
   - [UptimeRobot](https://uptimerobot.com/) - мониторинг каждые 5 минут
   - [Cron-Job.org](https://cron-job.org/) - отправка запросов каждые 50 секунд
   - [EasyCron](https://www.easycron.com/) - планировщик задач
   
   Настройте отправку GET запроса на: `https://qr-tg-authorization.onrender.com/health` каждые 50 секунд

2. **Или перейдите на платный тариф Render** ($7/месяц) для:
   - Постоянной работы без остановок
   - Мгновенного отклика
   - SSH доступа для отладки
...
- `SECRET_KEY` - любая случайная строка для безопасности

Без этих переменных QR-код не будет генерироваться!
